...
from .redis_tools import save_to_redis, load_from_redis, KEY_POLYGONS, KEY_GROUPS
...
def create_groups_of_polygons(polygons=None):
    ...
    if polygons is None:
        ...
        # Try Redis first
        cached_polygons = load_from_redis(KEY_POLYGONS)
        if cached_polygons:
             logger.info("AE: Loaded Polygons from Redis.")
             for p in cached_polygons:
                 # coords in Redis is list of lists, Shapely needs tuples
                 coords_tuples = [tuple(c) for c in p['coords']]
                 polygons.append({
                     'id': p['id'],
