...
from .redis_tools import save_to_redis, load_from_redis, KEY_WHITE_LINES, KEY_POLYGONS
...
def create_polygons(white_lines=None):
    ...
    if white_lines is None:
        ...
        # Try Redis first
        cached_white_lines = load_from_redis(KEY_WHITE_LINES)
        if cached_white_lines:
             logger.info("AD: Loaded White Lines from Redis.")
             for wl in cached_white_lines:
                 # Restore tuples from keys
                 start = tuple(wl['start']) # Redis stores as list
                 end = tuple(wl['end'])
                 path = [tuple(p) for p in wl['path']]
